...
import { highlightPanPath } from '@/server/curation/record';
import { fileExists } from '@/server/runtime';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

/** Phase 5: the panning flat clip for highlight n (Range support). 404 until rendered. */
async function serve(req: Request, id: string, nRaw: string): Promise<Response> {
  ...
  const p = highlightPanPath(item.id, Number(nRaw));
  if (!(await fileExists(p))) throw notFound('Pan clip not ready');
